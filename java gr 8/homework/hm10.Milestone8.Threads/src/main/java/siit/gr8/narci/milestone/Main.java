...
public class Main {
    ...
    public static void main ( String[] args ) throws IllegalArgumentException {
        ...
        Customer customer = new Customer ( "Narcisa Lontis", 70000, 1, "Volkswagen",
                "e-UP", true, true, true );
        ...
        CarRentalService carRentalService = new CarRentalService ( "Volkswagen", "e-UP", true, 2000, 25000,
                1, 60, 90, true );
        ...
        customer.buyCarAtFullPrice ( (ArrayList <Car>) cars, dealership );

        System.out.println ( "\nFEV5 - Customer requests Green Bonus for an ev from the dealership\n" );
        customer.customerRequestForGreenBonus ( (ArrayList <Car>) cars, dealership );
        ...
        customer.buyWithGreenBonus ( (ArrayList <Car>) cars, dealership );
        ...
        carDataWriterReader.writeToCsv ( (ArrayList <Car>) cars, "cars.csv" );

        System.out.println ( "\nNFEV6-Define the algorithm for computing the renting price by using generic types\n\nCost per day for: "
                +carRentalService.getManufacturer ()+" "+carRentalService.getModel ()+" is "+carRentalService.getPrice () / 1000+"Euro" );
        carRentalService.getRentalCostPerDay ( cars, carRentalService );
        carRentalService.getRentalCost ( cars, numberOfDays );
        System.out.println ( "Price for "+carRentalService.getManufacturer ()+" "+carRentalService.getModel ()+" rented for "+numberOfDays+" days"+
                " is: "+carRentalService.getPrice () / 1000 * numberOfDays+"Euro" );

        System.out.println ( "\nNFEV5-This is a new request from a new dealership for the Green Bonus Program\n" );

        Thread thread1 = new Thread ( dealership );
        thread1.start ();
        Thread thread2 = new Thread ( dealership );
        thread2.start ();
        try {
            thread1.join ();
            thread2.join ();
